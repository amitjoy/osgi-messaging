...
/**
 * Builder for the {@link MessageContext} that is capable to handle direct acknowledging/rejection of messages.
 *
...
public interface AcknowledgeMessageContextBuilder extends MessageContextBuilder {
     ...
     *
     * It gets the {@link Message} as parameter. To trigger acknowledgement or rejection, callers can access
     * the {@link AcknowledgeHandler} using the {@link AcknowledgeMessageContext} of the message.
     *
     * This handler is only called, if no filter configuration is provided.
     *
     * If postAcknowledge or/and postReject consumers are defined, they will be called after this handler.
     *
    ...
    /**
     * A service with {@link java.util.function.Consumer} as interface and parameter type {@link Message}, that
     * matches the provided target filter, is called to do custom acknowledge logic.
     *
     * It gets the {@link Message} as parameter. To trigger acknowledgement or rejection, callers can access the
     * {@link AcknowledgeHandler} using the {@link AcknowledgeMessageContext} of the message.
     *
     ...
     *
     * If the test of the predicate is <code>true</code>, the message will be acknowledged, otherwise rejected using
    ...
    /**
     * A service with {@link java.util.function.Predicate} as interface and parameter type {@link Message}, that
     * matches the provided target filter, is called to test receiving message to either acknowledge or reject.
     *
     * If the test of the predicate is <code>true</code>, the message will be acknowledged, otherwise rejected using
     * the implementation specific logic.
     *
     * If postAcknowledge or/and postReject consumers are set, they will be called after that. The filter is called
     * before a handleAcknowledge definition, if provided.
     *
     ...
     *
     * That consumer is called either, if a acknowledge filter returns <code>true</code> or a acknowledge handler
     * calls the acknowledge method.
     *
     * Depending on the message's {@link AcknowledgeType}, this callback can handle acknowledgement or rejection.
     *
     ...
     * properties matches the provided target filter.
     *
